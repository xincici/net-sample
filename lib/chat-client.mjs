...
const logger = getLogger('chat-client');
...
export default class ChatClient {
  ...
  connect() {
    ...
    this.socket = net.createConnection(this.port, this.address, () => {
      logger.info(chalk.green(`connected to ${this.address}:${this.port}`));
    });
  ...
  addListener() {
    ...
    this.socket.on('close', () => {
      logger.warn(chalk.yellow('connection closed'));
      this.rl.close();
      process.exit(0);
    });
    this.socket.on('error', err => {
      logger.error(chalk.red('connection error:'), err.message);
      this.rl.close();
      process.exit(1);
    });
