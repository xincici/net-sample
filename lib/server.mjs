...
export default class Server {
  constructor(port, address, callback) {
    ...
    callback = callback ? callback : () => logger.info('server has started on', chalk.green(`${address}:${port}`));
    this.port = port;
  ...
  addListener() {
    ...
    event.addListener(EVENT_CLIENT_LEAVE, client => {
      logger.info(`${chalk.red(client.name)} leave the chat`);
  ...
  createServer() {
    return net.createServer(c => {
      ...
      logger.info(chalk.red(name), 'connect');
    });
  }
